...
import {
  CountryTopTracksResult,
  ListenerEntry,
  PaginatedResult,
  TrackEntry,
  HistoryTrackEntry,
} from './track.interface.js';
...
export class TopTracksService implements OnModuleInit {
  ...
  async getListenersByTrack(
    ...
    const members: string[] = await redis.smembers(
      `track:${trackId}:listeners`,
    );
  ...
  async getUserHistory(
    ...
    try {
        ...
        data = resultList
          .map((item) => {
            try {
              ...
              this.logger.warn(
                `Failed to parse history track entry for user ${userKey}: ${item}`,
                err instanceof Error ? err.message : String(err),
              );
              return null;
            }
      ...
      this.logger.error(
        `Error fetching user history for ${userKey} from Redis`,
        error,
      );
  ...
  private async computeTopTracks(): Promise<TrackEntry[]> {
    ...
    this.logger.debug(
      `[Redis SCAN] Found ${listenerKeys.length} listener key(s): ${JSON.stringify(listenerKeys)}`,
    );
    ...
    this.logger.debug(
      `[Redis SCARD] Raw pipeline results: ${JSON.stringify(scardResults)}`,
    );
    ...
    for (let i = 0; i < listenerKeys.length; i++) {
      ...
      if (err) {
        this.logger.warn(
          `[Redis SCARD] Error for key "${listenerKeys[i]}": ${err.message}`,
        );
        continue;
      }
    ...
    this.logger.debug(
      `[Redis] Pairs after SCARD filter: ${JSON.stringify(pairs)}`,
    );
    ...
    this.logger.debug(
      `[Redis HGETALL] Raw meta results: ${JSON.stringify(metaResults)}`,
    );

    // 6. Assemble the response array.
    const result: TrackEntry[] = [];
    for (let i = 0; i < topPairs.length; i++) {
      const [err, meta] = metaResults![i] as [
        Error | null,
        Record<string, string>,
      ];
      if (err) {
        this.logger.warn(
          `[Redis HGETALL] Error for trackId "${topPairs[i].trackId}": ${err.message}`,
        );
        continue;
      }
      if (!meta) {
        this.logger.warn(
          `[Redis HGETALL] No meta found for trackId "${topPairs[i].trackId}"`,
        );
        continue;
      }
    ...
    this.logger.debug(
      `[Redis] Final computed top tracks: ${JSON.stringify(result)}`,
    );
    return result;
  }
