  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: WebSocket) {
    ...
    client.on('message', (raw) => {
      void this.handleClientMessage(client, (raw as Buffer).toString());
    });
  }
    ...
    await Promise.all(
      [...uniqueCountries.keys()].map(async (country) => {
        try {
          uniqueCountries.set(
            country,
            await this.topTracksService.getCountryNo1(country),
          );
        } catch (err) {
  ...
  private async sendCountryNo1(
    ...
    try {
      ...
      this.sendError(
        client,
        `Failed to fetch #1 track for country: ${country}`,
      );
    }
  }
